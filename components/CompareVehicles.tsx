...
// Category filters with specific logic
...
const categoryFilters: CategoryFilter[] = [
  ...
  { 
    id: 'sedan', 
    label: 'Sedan',
    filter: (v) => v.category?.toLowerCase() === 'sedan'
  },
  { 
    id: 'suv', 
    label: 'SUV',
    filter: (v) => v.category?.toLowerCase() === 'suv'
  },
  { 
    id: 'sports', 
    label: 'Sports',
    filter: (v) => v.category?.toLowerCase() === 'sports' || v.category?.toLowerCase() === 'sport'
  },
  { 
    id: 'luxury', 
    label: 'Luxury',
    filter: (v) => v.luxury === true
  },
  { 
    id: 'electric', 
    label: 'Electric',
    filter: (v) => v.fuel_type?.toLowerCase() === 'electric' || v.engine_type?.toLowerCase() === 'electric' || v.engine_type?.toLowerCase() === 'hybrid'
  },
  { 
    id: 'truck', 
    label: 'Truck',
    filter: (v) => v.category?.toLowerCase() === 'truck'
  },
  { 
    id: 'van', 
    label: 'Van',
    filter: (v) => v.category?.toLowerCase() === 'van'
  },
  { 
    id: 'coupe', 
    label: 'Coupe',
    filter: (v) => v.category?.toLowerCase() === 'coupe'
  },
  { 
    id: 'convertible', 
    label: 'Convertible',
    filter: (v) => v.category?.toLowerCase() === 'convertible'
  },
]
...
export default function CompareVehicles({ className = '' }: CompareVehiclesProps) {
  ...
  // Filter vehicles based on selected category, search, and sort
  const getFilteredVehicles = () => {
    ...
    if (categoryFilter && selectedCategory !== 'all') {
      filtered = filtered.filter(categoryFilter.filter)
    }
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(vehicle => 
        ...
        vehicle.car_code?.toLowerCase().includes(query)
      )
    }
  ...
  const getConditionBadge = (condition: string) => {
    ...
    if (cond === 'brand new') return { label: 'New', color: 'bg-green-500/80' }
    if (cond === 'foreign used') return { label: 'F-Used', color: 'bg-yellow-500/80' }
    if (cond === 'local used') return { label: 'L-Used', color: 'bg-blue-500/80' }
    return { label: 'Used', color: 'bg-gray-500/80' }
  }
  ...
  return (
    <div className={className}>
      ...
      {/* Search & Filters - Same horizontal layout as before */}
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <select
          ...
          className="px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white text-sm focus:outline-none focus:border-red-500/50 transition-colors appearance-none flex-shrink-0"
        >
          {sortOptions.map(opt => (
      ...
      {/* Results count - Added below the search bar */}
      <div className="flex items-center gap-2 mb-4">
        <span className="text-xs text-white/30">
          Showing <span className="text-white/60 font-medium">{filteredVehicles.length}</span> vehicles
          {selectedCategory !== 'all' && (
            <span className="text-white/40"> in <span className="text-white/60 font-medium">{categoryFilters.find(c => c.id === selectedCategory)?.label}</span></span>
          )}
        </span>
        {selectedCategory !== 'all' && (
          <button
      ...
      {filteredVehicles.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-white/40">
            {selectedCategory !== 'all' 
              ? `No ${categoryFilters.find(c => c.id === selectedCategory)?.label} vehicles found matching your criteria`
