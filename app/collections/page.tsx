...
import { Filter, Grid, List, ChevronDown, Loader2, AlertCircle, X, Search, Menu, Car, Crown, Zap, LayoutGrid } from 'lucide-react'
...
const STORAGE_KEYS = {
  VIEW_MODE: 'collection_view_mode',
...
export default function CollectionsPage() {
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      localStorage.setItem('collection_last_search', search)
    } else {
      const lastSearch = localStorage.getItem('collection_last_search')
      if (lastSearch) {
  ...
  // Fetch collection vehicles from Supabase
  useEffect(() => {
    const fetchCollectionVehicles = async () => {
      try {
        setDebugInfo('Fetching collection vehicles...')
        console.log('🔍 Fetching collection vehicles from Supabase...')
        ...
        if (error) {
          ...
          setError(`Failed to load collection vehicles: ${error.message}`)
  ...
  const clearSearch = () => {
    setSearchQuery('')
    localStorage.removeItem('collection_last_search')
  }
    ...
    return (
      <div className="min-h-screen bg-black">
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-8rem)]">
          ...
          <span className="text-white/60 ml-3">Loading our collection vehicles...</span>
        </div>
        <BottomNav />
      </div>
    )
      ...
      <main className="pb-24 md:pb-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Our Collections</h1>
              <p className="text-xs text-white/40">Curated collections of exceptional vehicles</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            {/* Categories Dropdown - Fixed width, smaller on mobile */}
          ...
          {vehicles.length === 0 ? (
            ...
            <div className="text-center py-12">
              <p className="text-white/40 text-sm">
                {searchQuery 
                  ? `No vehicles found in our collection matching "${searchQuery}"`
                  : 'No vehicles found in our collection matching your criteria'
                }
              </p>
            </div>
          ) : (
