...
// Category dropdown options - Sports page (exclude Sports)
const categories = [
...
const STORAGE_KEYS = {
  VIEW_MODE: 'sports_view_mode',
  SORT_BY: 'sports_sort_by',
  CONDITION: 'sports_condition'
}
...
function SportsContent() {
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      localStorage.setItem('sports_last_search', search)
    } else {
      const lastSearch = localStorage.getItem('sports_last_search')
      if (lastSearch) {
  ...
  // Fetch sports vehicles from Supabase
  useEffect(() => {
    const fetchSportsVehicles = async () => {
      try {
        setDebugInfo('Fetching sports vehicles...')
        console.log('🔍 Fetching sports vehicles from Supabase...')
        ...
        if (error) {
          console.error('❌ Error fetching sports vehicles:', error)
          setError(`Failed to load sports vehicles: ${error.message}`)
  ...
  const clearSearch = () => {
    setSearchQuery('')
    localStorage.removeItem('sports_last_search')
  }
    ...
    return (
      <div className="min-h-screen bg-black">
        ...
        <div className="flex items-center justify-center min-h-[calc(100vh-8rem)]">
          ...
          <span className="text-white/60 ml-3">Loading sports vehicles...</span>
        </div>
        <BottomNav />
      </div>
    )
      ...
      <main className="pb-24 md:pb-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Sports Cars</h1>
              <p className="text-xs text-white/40">High-performance vehicles engineered for speed</p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            <div className="flex-1 relative">
              <div className="flex items-center bg-white/5 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 border border-white/10 focus-within:border-red-500/50 transition-colors">
                ...
                <input
                  type="text"
                  placeholder="Search sports cars..."
                  value={searchQuery}
          ...
          {vehicles.length === 0 ? (
            <div className="text-center py-12">
              ...
              <div className="text-white/40 text-sm mb-2">No sports vehicles available</div>
              {debugInfo && (
            ...
            <div className="text-center py-12">
              <p className="text-white/40 text-sm">
                {searchQuery 
                  ? `No sports vehicles found matching "${searchQuery}"`
                  : 'No sports vehicles found matching your criteria'
                }
              </p>
            </div>
          ) : (
